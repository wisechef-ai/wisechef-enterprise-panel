...
- Running a Python script that calls the WiseChef Panel API
...
1. WiseChef Panel spawns the configured command as a child process
2. Standard WiseChef Panel environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the WiseChef Panel API and perform work.
