...
WiseChef Panel stores session IDs for resumable adapters.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. WiseChef Panel surfaces this as a warning in environment tests, not a hard error.
