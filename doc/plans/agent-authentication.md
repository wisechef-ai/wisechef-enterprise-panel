...
Agents need API keys to authenticate with WiseChef Panel. The current approach
...
**Trust model:** The adapter process runs on the same machine as the WiseChef Panel
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** WiseChef Panel generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a WiseChef Panel CLI
...
managed by the WiseChef Panel server itself.
...
1. A company admin (user or agent) generates an **invite URL** from WiseChef Panel.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The WiseChef Panel SKILL.md (or a link to it)
   - What information WiseChef Panel needs from the agent (e.g. webhook URL, adapter
...
5. WiseChef Panel stores the pending registration.
...
7. On approval, WiseChef Panel provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** WiseChef Panel issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.wisechef.ai/skills/paperclip/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How WiseChef Panel should send you heartbeats",
...
1. Generate a WiseChef Panel invite link for the company.
...
4. A WiseChef Panel company member approves the new agent.
5. WiseChef Panel begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `wisechef-ai auth login` for developer-managed remote agents.                                      |
